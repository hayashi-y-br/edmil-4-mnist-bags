...
from dataset import FourMNISTBags
...
def main(cfg: DictConfig):
    ...
    group = HydraConfig.get().job.override_dirname.replace('model.', '').replace('settings.', '')
    name = f'{group},seed={cfg.seed}'
    run = wandb.init(
        project='4-MNIST-Bags',
        name=name,
        group=group,
    ...
    elif cfg.model.name == 'AddMIL':
        model = AddMIL(num_classes=cfg.model.num_classes)
    if cfg.use_cuda:
        model.cuda()
